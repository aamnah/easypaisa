...
class ControllerExtensionPaymentEasypaisa extends Controller {
	...
	public function index() {
		...
		$data['entry_easypaisa'] = $this->language->get('entry_easypaisa');
		...
		foreach ($languages as $language) {
			if (isset($this->error['account' . $language['language_id']])) {
				$data['error_easypaisa' . $language['language_id']] = $this->error['account' . $language['language_id']];
			} else {
				$data['error_easypaisa' . $language['language_id']] = '';
			}
		}
		...
		foreach ($languages as $language) {
			if (isset($this->request->post['easypaisa_account' . $language['language_id']])) {
				$data['easypaisa_account' . $language['language_id']] = $this->request->post['easypaisa_account' . $language['language_id']];
			} else {
				$data['easypaisa_account' . $language['language_id']] = $this->config->get('easypaisa_account' . $language['language_id']);
			}
		}
		...
		$this->response->setOutput($this->load->view('extension/payment/easypaisa', $data));
	}

	protected function validate() {
		if (!$this->user->hasPermission('modify', 'extension/payment/easypaisa')) {
		...
		foreach ($languages as $language) {
			if (empty($this->request->post['easypaisa_account' . $language['language_id']])) {
				$this->error['account' .  $language['language_id']] = $this->language->get('error_easypaisa');
			}
		}
